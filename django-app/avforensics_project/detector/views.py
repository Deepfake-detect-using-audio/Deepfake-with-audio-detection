...
from .metadata_analysis import make_check

from .utils.face_cropper import extract_faces
...
def home(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            face_dir   = os.path.join(settings.MEDIA_ROOT, 'faces', video_name)
            extract_faces(file_path, face_dir)

            # Build URLs for the template
            face_images = [
                f"media/faces/{video_name}/face_{i+1}.jpg"
                for i in range(5)
                if os.path.exists(os.path.join(face_dir, f'face_{i+1}.jpg'))
            ]
            ...
            if score is None:
                ...
                if exception_occured == 0:
                    if make_flag == 1 and score < 50:
                        ...
                        result = "Most Likely Real!"
                    elif make_flag == 1 and score > 50:
                        result = "Possibly Real!"
                    elif make_flag == 0 and score > 50:
                        result = "Most Likely Fake!"
                    elif make_flag == 0 and score < 50:
                        result = "Possibly Real!"
                else:
                    result = "Most Likely Fake!" if score > 50 else "Most Likely Real!"
